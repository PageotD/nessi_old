...
def image(object, **options):
    ...
    if dt == 0 and object.header[0]['trid'] == 0:
...
def wiggle(object, **options):
    ...
    x1beg = options.get('x1beg', 0.)
    x1end = options.get('x1end', 0.)
...
def graph(object, **options):
    ...
    label2 = options.get('label2', ' ')
    title = options.get('title', ' ')
    tracecolor = options.get('tracecolor', None)
    tracestyle = options.get('tracestyle', '-')
    tracewidth = options.get('tracewidth', 2)
    skip = options.get('skip', 1)
    xcur = options.get('xcur', 1)
    ...
    if(a == None):
        for itrac in range(0, ntrac):
            if ndim == 1:
                wig = object.traces[:]*float(skip-1)*xcur
            else:
                if skip > 1:
                    wig = object.traces[itrac,:]*float(skip-1)*xcur
                else:
                    wig = object.traces[itrac,:]*xcur
            if tracecolor == None:
                plt.plot(wig, y, linestyle=tracestyle, linewidth=tracewidth)
            else:
                plt.plot(wig, y, color=tracecolor, linestyle=tracestyle, linewidth=tracewidth)
        ...
        if ndim == 1:
            ...
            if skip > 1:
                wig = object.traces[a,:]*float(skip-1)*xcur
            else:
                wig = object.traces[a,:]*xcur
        if tracecolor == None:
            plt.plot(wig, y, linestyle=tracestyle, linewidth=tracewidth)
        else:
            plt.plot(wig, y, color=tracecolor, linestyle=tracestyle, linewidth=tracewidth)