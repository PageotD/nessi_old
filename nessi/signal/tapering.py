...
def _linear(n, ntap1, ntap2, datatype=np.float32):
    """
    Linear taper type.
    ...
    ftap = np.zeros(n, dtype=datatype)
    ftap[:] = 1
...
def _sine(n, ntap1, ntap2, datatype=np.float32):
    """
    Sine taper type.
...
def _cosine(n, ntap1, ntap2, datatype=np.float32):
    """
    Cosine taper type.
...
def time_taper(data, dt=0.01, **options):
    ...
    if type == 'linear':
        ftap = _linear(ns, ntap1, ntap2, datatype=datatype)
    if type == 'sine':
        ftap = _sine(ns, ntap1, ntap2, datatype=datatype)
    if type == 'cosine':
        ftap = _cosine(ns, ntap1, ntap2, datatype=datatype)
